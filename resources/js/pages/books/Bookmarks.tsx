  ...
  const handleRemove = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      if (res.data.status === 'removed' || res.data.status === 'success') {
        onRemove(id);
        // [TRANSLATE]
        setNotification(t("Book removed from bookmarks"));
      }
    } catch (err) {
      console.error(err);
    } finally {
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-6 pt-32">
          ...
          <div className="flex flex-col md:flex-row justify-between items-end gap-8 mb-12">
            <div className="space-y-3 flex-1">
              ...
              >
                <Icons.StarSolid />
                {/* [TRANSLATE] */}
                <span>{t("Personal Collection")}</span>
              </motion.div>
              ...
              >
                {/* [TRANSLATE] */}
                {t("Bookmarks")} <span className="text-transparent bg-clip-text bg-gradient-to-r from-yellow-500 to-orange-600">{t("Your")}</span>
              </motion.h1>
              ...
              >
                {/* [TRANSLATE] */}
                {localBooks.length} {t("books saved to read later.")}
              </motion.p>
            </div>
            ...
            {localBooks.length > 0 && (
              ...
              >
                ...
                <input 
                  ...
                  // [TRANSLATE]
                  placeholder={t("Search in bookmarks...")}
          ...
          {localBooks.length === 0 && (
            ...
            >
              ...
              <p className="text-gray-500 dark:text-gray-400 max-w-md mb-8 leading-relaxed">
                {/* [TRANSLATE] */}
                {t("It seems you haven't saved any books yet. Explore our collection and save your favorites here.")}
              </p>
              <Link
                href="/books"
          ...
          {localBooks.length > 0 && filteredBooks.length === 0 && (
            <div className="text-center py-20">
              {/* [TRANSLATE] */}
              <p className="text-gray-500 text-lg">{t("No books found matching")} "{search}"</p>
              {/* [TRANSLATE] */}
              <button onClick={() => setSearch("")} className="mt-2 text-yellow-600 font-medium hover:underline">{t("Reset Search")}</button>
            </div>
          )}
