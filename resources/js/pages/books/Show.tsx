...
const Icons = {
  Bookmark: ({ solid }: { solid?: boolean }) => (
    <svg className={`w-6 h-6 transition-all duration-300 ${solid ? "text-red-500 fill-red-500 scale-125 transform" : "text-white/80 hover:text-white/90"}`} fill={solid ? "currentColor" : "none"} viewBox="0 0 24 24" stroke="currentColor">
  ...
  Star: ({ filled }: { filled?: boolean }) => (
    <svg className={`w-8 h-8 transition-colors duration-200 ${filled ? "text-yellow-400 fill-yellow-400" : "text-gray-300 dark:text-gray-600"}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
      <path strokeLinecap="round" strokeLinejoin="round" d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.838 5.666h5.968c.969 0 1.371 1.24.588 1.81l-4.825 3.511 1.838 5.666c.3.921-.755 1.688-1.542 1.115l-4.825-3.511-4.825 3.511c-.787.573-1.842-.194-1.542-1.115l1.838-5.666-4.825-3.511c-.783-.57-.381-1.81.588-1.81h5.968l1.838-5.666z" />
    </svg>
  ),
};
...
const BookmarkButton = ({ book, user }: { book: Book; user: any }) => {
  ...
  const toggle = async () => {
    if (!user) {
      // [TRANSLATE]
      if (confirm(t("Please login to save bookmarks."))) {
    ...
    try {
      ...
      // [TRANSLATE]
      alert(t("Failed to save bookmark. Please try again."));
    }
    setLoading(false);
  };
...
const RatingWidget = ({ bookId, initialUserRating, avgRating, user }: { bookId: number, initialUserRating: number | null, avgRating: number | null, user: any }) => {
  ...
  const handleRating = async (value: number) => {
    if (!user) {
      // [TRANSLATE]
      if (confirm(t("Please login to rate."))) {
        window.location.href = "/login";
      }
      return;
    }
    ...
    try {
      ...
      // [TRANSLATE]
      alert(t("An error occurred while saving the rating."));
    } finally {
  ...
  return (
    <div className="flex flex-col gap-2 mt-6 p-4 bg-gray-50 dark:bg-gray-800/50 rounded-2xl border border-gray-100 dark:border-gray-700/50">
      ...
      {/* [TRANSLATE] */}
      {isSubmitting && <span className="text-xs text-indigo-500 animate-pulse">{t("Saving...")}</span>}
    </div>
  );
};
          ...
          >
            ...
            <div>
              ...
              <p className="text-gray-700 dark:text-gray-300 whitespace-pre-line leading-relaxed text-base">
                {/* [TRANSLATE] */}
                {book.description || t("No description available for this book.")}
              </p>
            </div>
            ...
            <div className="pt-4 flex flex-col sm:flex-row gap-4 items-start sm:items-center">
              ...
              >
                <svg className="w-5 h-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 6.253v13m0-13C10.832 5.409 9.175 5 7.5 5 4.582 5 2 7.238 2 10c0 2.239 2.07 4.102 5.093 4.551A9 9 0 0012 20.25c2.474 0 4.792-.612 6.807-1.748C20.897 18.156 22 16.29 22 14c0-2.762-2.582-5-5.5-5-.466 0-.916.05-1.353.14M12 6.253v13" />
                </svg>
                {/* [TRANSLATE] */}
                {t("Start Reading")}
              </Link>
        ...
        {relatedBooks.length > 0 && (
          <div className="mt-20">
            <h2 className="text-3xl font-bold text-gray-900 dark:text-white mb-8 border-b-2 border-indigo-500/50 pb-2">
              {/* [TRANSLATE] */}
              📚 {t("Other Books in this Category")}
            </h2>
                  ...
                  >
                    <Link href={`/books/${b.id}`}>
                      <div className="aspect-[2/3] rounded-xl overflow-hidden bg-gray-200 dark:bg-gray-800 border border-gray-100 dark:border-gray-700">
                        <img
                          src={b.cover_path && b.cover_path.startsWith("http") ? b.cover_path : `/storage/${b.cover_path}`}
            ...
            {filtered.length === 0 && (
              ...
              >
                {/* [TRANSLATE] */}
                🔍 {t("No related books found matching")} **"{searchRelated}"**.
              </motion.p>
            )}
          </div>
        )}
